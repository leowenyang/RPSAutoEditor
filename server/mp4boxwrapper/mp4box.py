...
class Output(ParameterContainer):
    ...
    def __iter__(self):
        return chain(ParameterContainer.__iter__(self), ['-out', self.file_path])


class MP4boxProcess(object):
    """Class to exectute MP4box.
...
class MP4box(ParameterContainer):
    """This class represents the MP4box command.
    ...
    def __init__(self, binary='mp4box', *args):
        self.binary = binary
        self.process = None
        ParameterContainer.__init__(self, *args)
    ...
    def run(self):
        ...
        :class:`MP4boxProcess` object which have already the
        :meth:`MP4boxProcess.run` invoked.

        :return: :class:`MP4boxProcess` object with `run()` invoked
        """
        return MP4boxProcess(self).run()
